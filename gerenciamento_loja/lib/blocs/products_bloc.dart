...
class ProductBloc extends BlocBase {
  ...
  Future<bool> saveProduct() async {
    ...
    try {
      if(product != null) {
        ...
        DocumentReference dr = await FirebaseFirestore.instance.collection("products").doc(categoryId).collection("items").add(Map.from(unsavedData)..remove("images"));
  ...
  Future _uploadImages(String productId) async {
    for(int i = 0; i < unsavedData["images"].length; i++) {
      ...
      UploadTask uploadTask = FirebaseStorage.instance.ref().child(categoryId).child(productId).child(
